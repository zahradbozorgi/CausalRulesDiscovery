""" Function for modelling uplift trees
"""
...
def create_uplift_tree(df, r_copy2):
    """ Function for training uplift trees

    :param df: dataframe containg rules
    :param r_copy2: list of rules
    """
    for rule in r_copy2:
        print(rule)
        ...
        features = df_train.drop(['Selected', 'Treatment'], axis=1)
        ...
        uplift_tree_string(uplift_model.fitted_uplift_tree, features.columns.values.tolist())

        # save uplift tree as png
        graph = uplift_tree_plot(uplift_model.fitted_uplift_tree, features.columns.values.tolist())
        with open("../reports/figures/tree2_%s.png" % r_copy2.index(rule), "wb") as png:
